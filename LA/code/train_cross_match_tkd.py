...
# ablation study Ltkd
parser.add_argument('--use_pww', action='store_true', help='include p^w_w in Ltkd')
parser.add_argument('--use_pws', action='store_true', help='include p^w_s in Ltkd')
parser.add_argument('--use_psw', action='store_true', help='include p^s_w in Ltkd')
parser.add_argument('--use_pss', action='store_true', help='include p^s_s in Ltkd')
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (
            ...
            # ablation study Ltkd
            tkd_terms = []
            if args.use_pww: tkd_terms.append(loss_weak_dec_w_t)
            if args.use_pws: tkd_terms.append(loss_strong_dec_w_t)
            if args.use_psw: tkd_terms.append(loss_weak_dec_w_s2_t)
            if args.use_pss: tkd_terms.append(loss_strong_dec_w_s2_t)

            if tkd_terms:
                loss_tkd_mean = sum(tkd_terms)/len(tkd_terms)
            else:
                loss_tkd_mean = torch.tensor(0.0, device=loss_x.device)   

            loss_u_w_kd = (1-eta)*loss_tkd_mean + eta*(loss_weak_dec_w_s2+loss_strong_dec_w_s2)/2
